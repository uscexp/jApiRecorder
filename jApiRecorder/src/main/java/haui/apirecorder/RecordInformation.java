...
public class RecordInformation {
	...
	private void validateArguments(String methodName, Object[] args, int[] argIdx4Pk) {
		...
		if(argIdx4Pk == null || argIdx4Pk.length <= 0) {
			throw new IllegalArgumentException(String.format("Parameter %s must have at least one value!", "argIdx4Pk"));
		}
		...
		for (int i = 0; i < argIdx4Pk.length; i++) {
			...
			if(argIdx4Pk[i] >= args.length) {
				throw new IllegalArgumentException(String.format("The index %d in array %s can not be larger than length of array %s!", i, "argIdx4Pk", "args"));
			}
		}
	}
